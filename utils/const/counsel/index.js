export const transparentCounselData = {
  designs: {
    title: [
      {
        header: 'Responsive, Transparent Counsel',
        body: 'From design applications to registration and enforcement, we can help you establish robust design rights.',
      },
    ],
    header: [
      {
        title: 'Protect your valuable design.',
        body:
          'Your product’s unique aesthetic makes it more desirable in-market when compared with competing products.\n\n' +
          ' A design registration protects your valuable differentiating aesthetics from competitors who might want to leverage off your design work to benefit from it or create confusion with your product. A registered design provides 10 years of design ownership in the countries you choose.',
      },
    ],
    data: [
      {
        icon: '/counsel/Patents Search.svg',
        title: 'Protect the "look" of your product',
        body: 'Does your design look better than competitors’ designs? We can file a registered design to protect that business advantage.',
      },
      {
        icon: '/counsel/Patent How To.svg',
        title: 'Prepare and file your design application',
        body: 'Send us images of your design or we can have them prepared. We can then have your design application filed within the week.',
      },
      {
        icon: '/counsel/Patent Litigation.svg',
        title: 'Stop design infringement',
        body: 'We can advise other parties to cease and desist copying on the basis of your registered design.',
      },
    ],
  },
  'ip-oppositions-disputes': {
    title: [
      {
        header: 'Strong & Efficient Counsel',
        body: 'From determining IP infringement and sending a cease and desist letter through to a patent opposition, trade mark opposition or managing a dispute, our experienced IP attorneys are ready to guide you.',
      },
    ],
    header: [
      {
        title: 'Resolve your IP dispute.',
        body:
          'Whether your dispute requires strategic negotiation, a cease and desist letter, mediation, arbitration, opposition or, in consultation with our legal team, Federal court enforcement we’re ready to help you identify options for resolution.',
      },
    ],
    data: [
      {
        icon: '/counsel/Patents Search.svg',
        title: 'Assess infringement, send cease and desist letters',
        body: 'We will articulate the strength of your position and attempt to negotiate a settlement that achieves your commercial objectives.',
      },
      {
        icon: '/counsel/Patent How To.svg',
        title: 'Lodge a patent opposition',
        body: 'Challenge an accepted patent or trade mark application.',
      },
      {
        icon: '/counsel/Patent Litigation.svg',
        title: 'Enforce your patent',
        body: 'Whether through negotiation, alternative dispute resolution or Federal Court litigation, with our legal team.',
      },
    ],
  },
  'ip-strategy': {
    title: [
      {
        header: 'Thorough Counsel, Actionable Insights',
        body: '',
      },
    ],
    header: [
      {
        title: 'Know your IP. Understand its value. Maximise its potential.',
        body:
          'Establishing a clear understanding of the underlying intellectual property in a business is the first step to understanding its commercial value to the business or third party and how its potential can be maximised. IP audit, valuation and due diligence processes can be conducted on the identifiable IP of any business and we’re ready to assist.',
      },
    ],
    data: [
      {
        icon: '/counsel/Patents Search.svg',
        title: 'Audit your IP',
        body: 'Distill your intangible assets, such as inventions, brands, trade dress, production techniques, trade secrets (e.g. formulas, methods or processes) into measurable, valuable and manageable forms.',
      },
      {
        icon: '/counsel/Patent How To.svg',
        title: 'Conduct IP Due Diligence',
        body: 'Conduct due diligence on a company’s IP, its ownership, validity and SWOT for a safer investment, merger or acquisition.',
      },
      {
        icon: '/counsel/Patent Litigation.svg',
        title: 'Value your IP',
        body: 'We can refer you to companies that conduct IP valuation analysis for strategic planning, M&A activities, CGT calculation, JVs or in justification of tax returns.',
      },
    ],
  },
  'patent-oppositions-disputes': {
    title: [
      {
        header: 'Strong & Efficient Counsel',
        body: 'From determining patent infringement and sending a cease and desist letter through to a patent opposition or negotiating a dispute, our experienced patent attorneys are ready to guide you.',
      },
    ],
    header: [
      {
        title: 'Resolve your patent dispute.',
        body:
          'Whether your dispute requires strategic negotiation, a cease and desist letter, mediation, arbitration, opposition or, in consultation with our legal team, Federal court enforcement we’re ready to help you identify options for resolution.',
      },
    ],
    data: [
      {
        icon: '/counsel/Patents Search.svg',
        title: 'Assess infringement, send cease and desist letters',
        body: 'We will articulate the strength of your position and attempt to negotiate a settlement that achieves your commercial objectives.',
      },
      {
        icon: '/counsel/Patent How To.svg',
        title: 'Lodge a patent opposition',
        body: 'Challenge an accepted patent or trade mark application.',
      },
      {
        icon: '/counsel/Patent Litigation.svg',
        title: 'Enforce your patent',
        body: 'Whether through negotiation, alternative dispute resolution or Federal Court litigation, with our legal team.',
      },
    ],
  },
  patents: {
    title: [
      {
        header: 'Responsive, Transparent Counsel',
        body: 'From patent searches and applications to handling disputes and litigation, we can help you establish robust patent rights.',
      },
    ],
    header: [
      {
        title: 'Protect your valuable idea to secure your business.',
        body:
          'Your intellectual property is what makes your product unique. By lodging a patent application, your patent pending status will establish your claim on your new idea, provide investors with confidence and help deter competitors from copying.\n\n' +
          'A granted patent provides ownership of an invention for up to 20 years in the countries you choose and provides both monopoly and licensing rights.',
      },
    ],
    data: [
      {
        icon: '/counsel/Patents Search.svg',
        title: 'Clearly delineate your patent claims',
        body: 'Iterate towards the perfect set of claims that cover your commercial products and delineate your ideas from the prior art.',
      },
      {
        icon: '/counsel/Patent How To.svg',
        title: 'Enforce your patent rights',
        body: 'Patent ownership allows you to stop competitors from copying your patented product for 20 years.',
      },
      {
        icon: '/counsel/Patent Litigation.svg',
        title: 'Prepare and file your patent application',
        body: 'Our team can prepare your patent application and have it filed in any country around the world.',
      },
    ],
  },
  'trade-mark-oppositions-disputes': {
    title: [
      {
        header: 'Strong & Efficient Counsel',
        body: 'From sending a cease and desist letter, to obtaining consent orders, filing a trade mark opposition or negotiating a trade mark dispute, our experienced trade mark attorneys can guide you to the best possible outcomes.',
      },
    ],
    header: [
      {
        title: 'Resolve your trade mark dispute.',
        body:
          'Whether your dispute requires strategic negotiation, a cease and desist letter, mediation, arbitration, consent orders, opposition or court enforcement, we’re ready to help you identify options for resolution.',
      },
    ],
    data: [
      {
        icon: '/counsel/Patents Search.svg',
        title: 'Send or defend cease and desist letters',
        body: 'We will articulate the strength of your position and attempt to negotiate a settlement that achieves your commercial objectives.',
      },
      {
        icon: '/counsel/Patent How To.svg',
        title: 'Lodge a trade mark opposition',
        body: 'Challenge an accepted trade mark application.',
      },
      {
        icon: '/counsel/Patent Litigation.svg',
        title: 'Enforce your trade mark',
        body: 'Whether through negotiation, alternative dispute resolution or Federal Circuit Court litigation, with our legal team.',
      },
    ],
  },
  'trade-marks': {
    title: [
      {
        header: 'Responsive, Transparent Counsel',
        body: 'From trade mark filing and trade mark management to overcoming disputes and litigation, we can help you maximise the benefits of your registered trade mark.',
      },
    ],
    header: [
      {
        title: 'Protect your trade mark to secure your brand’s future.',
        body:
          'Your trade mark is the face of your business. It directs customers back to your company, the source of the quality of your products or services. A successful trade mark is essential to building brand loyalty and reputation. To stand out from the crowd, get in touch with us today.',
      },
    ],
    data: [
      {
        icon: '/counsel/Patents Search.svg',
        title: 'Advise you in establishing your brand',
        body: 'What message you convey to your market and what distinguishes your brand from others.',
      },
      {
        icon: '/counsel/Patent How To.svg',
        title: 'Prepare and file your trade mark application',
        body: 'Make pre-filing checks to give your trade mark application the best chance of success.',
      },
      {
        icon: '/counsel/Patent Litigation.svg',
        title: 'Enforce your trade mark',
        body: 'Defend your brand from trade mark infringers. Keep your goodwill.',
      },
    ],
  },
}
